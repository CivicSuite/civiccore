"""PostgreSQL-backed local task queue for Townlight desktop runtimes."""
...
class LocalTask(Base):
    ...
    __tablename__ intentionally keeps its pre-rename literal value
    (``civiccore_local_tasks``) because it is a real Postgres table name
    that already exists in every consuming module's database; renaming it
    here would orphan already-deployed local task queues.
    """
