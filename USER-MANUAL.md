...
2. **Technical guide for module developers and IT integrators** — install,
   ...
   alembic chain.
3. **Architecture reference** — submodules, the migration runner, the
   prompt-template engine, the provider registry, context utilities, and
   structured output, with a text diagram of how they fit together.
...
Rather than each of those apps re-inventing the same plumbing —
authentication, audit logging, LLM provider integration, document
ingestion, prompt templates, model registries, sovereignty verification —
CivicCore provides one shared, audited, semver-stable implementation that
each module imports.
