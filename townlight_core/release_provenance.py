"""Release provenance checks shared across Townlight modules.
...
Under the Townlight v1 provenance model, tags are release pointers and the
Sigstore-signed release attestation is the trust artifact.

NOTE (CivicSuite -> Townlight rebrand): the default ``--repo`` value and the
GitHub API ``User-Agent`` string below (both literally "civiccore") are
intentionally NOT renamed. tests/test_release_provenance.py and the real,
already-published release history (v0.1.0 - v1.2.1) pin these values, and the
"CivicSuite/civiccore" identity is cryptographically embedded in already-signed
Sigstore attestations for those releases — changing it here would not change
history and would break verification of the exact identity real signatures
assert. See the PR description for the full rationale.
"""
...
def canonical_json_bytes(payload: dict[str, Any]) -> bytes:
    """Return Townlight's canonical JSON serialization for attestations."""
...
def _require_attestation_shape(attestation: dict[str, Any]) -> None:
    if attestation.get("schema_version") != ATTESTATION_SCHEMA_VERSION:
        raise ProvenanceError(
            ...
            "versioned Townlight Core attestation contract."
        )
