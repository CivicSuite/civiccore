"""Alembic env for townlight_core. Reads the connection from Config.attributes['connection'] when invoked by runner.py (the normal path). Falls back to DATABASE_URL env var when invoked standalone.

Note: VERSION_TABLE below intentionally keeps its pre-rename literal name
(``alembic_version_civiccore``) because it is a real Postgres table name that
already exists in every consuming module's database. Renaming this constant's
value would orphan existing deployments' migration history; only the import
path and generic prose in this package were renamed for the CivicSuite ->
Townlight rebrand."""
