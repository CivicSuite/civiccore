"""CLI worker for the Townlight Core PostgreSQL-backed local task queue."""
...
from townlight_core.platform.task_queue import run_one_local_task
from townlight_core.platform.tasks import TaskRetryPolicy
from townlight_core.tasks.registry import get_task_handlers
...
async def _run_worker(args: argparse.Namespace) -> int:
    ...
    if not database_url:
        raise RuntimeError("DATABASE_URL is required for townlight_core.tasks.worker")
    _load_handler_modules(os.environ.get("TOWNLIGHT_CORE_TASK_HANDLER_MODULES"))
...
def main(argv: Sequence[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Run the Townlight Core local task worker.")
