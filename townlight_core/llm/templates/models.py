...
NOTE (CivicSuite -> Townlight rebrand): the ``consumer_app`` default below is
intentionally still the literal ``'civiccore'`` string. It matches the real
column-level DEFAULT already applied by the 0002_llm migration and the
`consumer_app='civiccore'` rows already persisted in every consuming
module's database; changing it here without a corresponding data migration
would silently break default-template resolution. See
``townlight_core/llm/templates/resolver.py``.
"""
...
class PromptTemplate(Base):
    ...
    # because townlight_core has no User ORM model — users.id lives in records-ai
    # and is not part of townlight_core.db.Base.metadata. Including the ORM-level
